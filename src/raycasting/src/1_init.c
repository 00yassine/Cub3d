...
void	init_data_from_map(t_data *data, t_map *map_data)
{
	...
	data->win_ptr = mlx_new_window(data->mlx_ptr, data->cols * TS, data->rows * TS, "cub3d");
	data->img = mlx_new_image(data->mlx_ptr, data->cols * TS, data->rows * TS);
	...
	data->last_ms = 0;
	data->input = (t_input){0, 0, 0, 0};
	draw_2d_map(data, data->img_data, data->line_len);
	draw_player(data, data->img_data, data->line_len);
